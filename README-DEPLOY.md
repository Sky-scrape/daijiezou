...
也可以把密钥写进项目根 `.env`(每行 `KEY=VALUE`,已被 `.gitignore` 的 `.env*` 排除),重启服务器自动加载;真实环境变量优先,部署平台配置的变量不受影响。
...
| D. 直答问答(看盘君) | `ZHIHU_ACCESS_SECRET` | 问答框保留,按「服务端 LLM → 知乎直答 → 本地规则」三级降级 |
...
1. 把 `daijiezou-demo` 文件夹推到 GitHub(⚠ 凭证只放环境变量或本地 `.env`;`.env*` 已被 `.gitignore` 排除,仓库里没有秘密——推之前用 `git status` 再确认一次)
...
> - **盐言故事内容 API**(免鉴权):拉取真实故事标题套路作为游戏内「雇写手」软文的风格参照——让玩家看到的每一篇"离职员工体"都有真实的社区文风出处;接口若返回作者则帖内保留归属标注(当前上游已不含作者字段,自动省略)。
