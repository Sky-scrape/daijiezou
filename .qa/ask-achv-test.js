...
console.log('[B] 回答守卫');
{
  ...
  st.ap = 0;
  check('无 AP 押注拒绝', answerAsk(st, 'long').ok === false);
  st.ap = 2;
  const rb = answerAsk(st, 'long');
  check('押注成功扣 1 AP', rb.ok === true && st.ap === 1);
  check('重复回答拒绝', answerAsk(st, 'short').ok === false && answerAsk(st, 'joke').ok === false);
  check('卡进入 bet 态', st.askPending.state === 'bet' && st.askPending.choice === 'long');
  st.askPending = null;
  const q2 = spawnAsk(st, 1);
  const rj = answerAsk(st, 'joke');
  check('抖机灵免费可用', rj.ok === true && st.ap === 1);
